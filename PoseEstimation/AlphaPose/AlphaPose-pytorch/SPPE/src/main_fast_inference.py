...
class InferenNet(nn.Module):
    def __init__(self, kernel_size, dataset):
        ...
        print('Loading pose model from {}'.format('./PoseEstimation/AlphaPose/AlphaPose-pytorch/models/sppe/duc_se.pth'))
        sys.stdout.flush()
        model.load_state_dict(torch.load('./PoseEstimation/AlphaPose/AlphaPose-pytorch/models/sppe/duc_se.pth'))
        model.eval()
        self.pyranet = model
